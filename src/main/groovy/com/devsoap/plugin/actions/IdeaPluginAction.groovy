    ...
    protected void executeAfterEvaluate(Project project) {
        ...
        if ( module.inheritOutputDirs == null ) {
            ...
            if ( runTask.classesDir == null ) {
                module.outputDir = Util.getMainSourceSet(project).destinationDirectory.asFile.getOrElse(new File(project.buildDir, "classes/kotlin/main"))
                module.testOutputDir = Util.getMainTestSourceSet(project).destinationDirectory.asFile.getOrElse(new File(project.buildDir, "classes/kotlin/test"))
            } else {
